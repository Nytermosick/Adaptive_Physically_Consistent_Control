...
class Simulator:
    ...
    def __init__(self, 
        ...
        self.path = f'logs/{datetime.now().strftime("%d_%m_%H_%M_%S")}'
        os.mkdir(self.path)
        self.video_path = Path(self.path + "/" + self.video_name)
    ...
    def plot_results(self):
        ...
        for i in range(self.pos_err.shape[1]):
            plt.plot(self.times, self.pos_err[:, i], label=f'Joint {i+1}')
        plt.xlabel('Time [s]')
        plt.ylabel('Joint Position Errors [rad]')
        plt.title('Joint Position Errors over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{self.path}/adaptive_control_position_errors.png')
        plt.close()
        ...
        # States plot
        plt.figure(figsize=(10, 6))
        for i in range(self.estimated.shape[1]):
            plt.plot(self.times, [self.real[i]] * self.estimated.shape[0] , label=parameters[i], color=colors[i], linestyle='--')
            plt.plot(self.times, self.estimated[:, i], label=parameters[i], color=colors[i])

        plt.xlabel('Time [s]')
        plt.ylabel('Value of parameter')
        plt.title('Value of real/estimated parameter over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{self.path}/adaptive_control_parameters.png')
        plt.close()

        # Esimated states plot
        plt.figure(figsize=(10, 6))
        for i in range(self.estimated.shape[1]):
            plt.plot(self.times, self.estimated[:, i], label=parameters[i])
        plt.xlabel('Time [s]')
        ...
        plt.savefig(f'{self.path}/adaptive_control_estimated_parameters.png')
        plt.close()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{self.path}/adaptive_control_control_signals.png')
        plt.close()